...
import { AddVideoType, AddVideo } from "@/app/(root)/assisted/_type";
import {
  FileInput,
  FileSvgDraw,
  FileUploader,
...
export default function AddVideoForm({ isEdit, info, handleClose, id }: Props) {
  const params = useParams();
  const [pendding, setPendding] = useTransition();
  const router = useRouter();
  const form = useForm<AddVideoType>({
    resolver: zodResolver(AddVideo),
    defaultValues: getDefaultValues(info),
  });

  function onSubmit(values: AddVideoType) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="sm:col-span-2">
            <FormField
              ...
              render={({ field }) => (
                <>
                  <FormLabel className="text-base">ڤیدۆ </FormLabel>
                  <FileUploader
...
const getDefaultValues = (values: Partial<AddVideoType> = {}) => {
