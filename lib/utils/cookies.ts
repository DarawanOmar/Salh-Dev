...
export async function login(token: string, redirectValues?: string) {
  ...
  (await cookies()).set("KhoshnawGroup", session, {
    expires,
    secure: false,
    httpOnly: true,
    path: "/",
...
export async function logout() {
  // Destroy the session
  (await cookies()).set("KhoshnawGroup", "", {
    expires: new Date(-1),
    secure: false,
  });
  redirect("/sign-in");
...
export async function getSession() {
  const session = (await cookies()).get("KhoshnawGroup")?.value;
  if (!session) return null;
  return await decrypt(session);
}

export async function updateSession(request: NextRequest) {
  const session = request.cookies.get("KhoshnawGroup")?.value;
